 ...
 */
class PixReviewsProcessorImpl implements PixReviewsProcessor {

	/** @var PixReviewsMeta plugin configuration */
	protected $meta = null;

	/** @var PixReviewsMeta field information */
	...
	/** @var PixReviewsMeta current data; including submitted data */
	protected $data = null;
		...
		try {
			...
			if ($this->form_was_submitted()) {
				...
				if (empty($errors)) {
					...
					$this->data = pixreviews::instance('PixReviewsMeta', $input);
	...
	protected function load_data_from_database($option_key) {
		...
		$this->data = pixreviews::instance('PixReviewsMeta', $dbconfig);
	}
	...
	protected function cleanup_input($input) {
		...
		foreach ($this->fields->metadata_array() as $key => $field) {
			...
			foreach ($cleanup as $rule) {
				$callback = pixreviews::callback($rule, $this->meta);
	...
	/**
	 * @return PixReviewsMeta current data (influenced by user submitted data)
	 */
	function data() {
		if ($this->status === null) {
			$this->run();
